...
def fetch_ads_settings(
    effective_statuses: Optional[List[str]] = None,
    ad_account_ids: Optional[List[str]] = None,
    max_workers: int = 1,
) -> List[Dict[str, Any]]:
    """
    Fetch ad metadata/settings across configured ad accounts.

    Args:
        effective_statuses: optional list to filter ads (e.g. ["ACTIVE", "PAUSED"])
    """
    ...
    base_params: Dict[str, Any] = {
        "fields": ",".join(ADS_SETTINGS_FIELDS),
        "limit": 200,
    }
        ...
        with ThreadPoolExecutor(max_workers=workers) as pool:
            ...
            for fut in as_completed(futures):
                try:
                    ...
                    raise GraphAPIError(f"Ads settings fetch failed for {futures[fut]}: {e}") from e
